...
def write_transformed_struct_vspec(
    ...
    if timestamp_schema and timestamp_schema.components:
        entries: dict[str, dict[str, object]] = {
            timestamp_schema.struct_name: dict(timestamp_schema.struct_entry),
        }
        for component in timestamp_schema.components:
            entries[f"{timestamp_schema.struct_name}.{component.name}"] = dict(component.vspec_entry)
    else:
        entries = {
            "Time_t": {
                "type": "struct",
    ...
    for node, data in sorted(leaves, key=lambda x: x[0].get_fqn()):
        ...
        if timestamp_schema and timestamp_schema.components:
            for component in timestamp_schema.components:
                entries[f"{fqn}.time.{component.name}"] = dict(component.vspec_entry)
        else:
